...
void CServerBrowser::RefreshInternetServers()
{
	...
	strncpy_safe( pFilters[ 0 ].m_szKey, "gameaddr", sizeof(pFilters[ 0 ].m_szKey) );
	strncpy_safe( pFilters[ 0 ].m_szValue, "144.76.94.38", sizeof(pFilters[ 0 ].m_szValue) );
	...
	m_hServerListRequest = SteamMatchmakingServers()->RequestInternetServerList(107410, &pFilter, ARRAYSIZE(pFilters), this );
	//m_hServerListRequest = SteamMatchmakingServers()->RequestHistoryServerList(107410, nullptr, ARRAYSIZE(pFilters), this);
}
...
void CServerBrowser::ServerResponded( HServerListRequest hReq, int iServer )
{
	...
	if ( pServer )
	{
		...
		if ( pServer->m_nAppID == SteamUtils()->GetAppID() )
		{
			...
			m_nServers++;
			ostringstream str;
			str << "Server " << iServer << " : " << pServer->GetName() << " | " << pServer->m_nPlayers << "/" << pServer->m_nMaxPlayers << endl;
			OutputDebugString(str.str().c_str());
			ostringstream constr;
			
			string ip(pServer->m_NetAdr.GetConnectionAddressString());
			stringstream ss(ip);

			getline(ss, ip, ':');
			constr << L"steam://connect/" << ip << "/";
			ShellExecute(0, 0, constr.str().c_str(), 0, 0, SW_SHOW);
			//constr << R"("R:\SteamLibrary\steamapps\common\Arma 3\arma3launcher.exe" -noLauncher -connect=)" << ip << " -port=2302";
			//OutputDebugString(constr.str().c_str());
			//OutputDebugString("\n");
			////WinExec(constr.str().c_str(),0);
			//STARTUPINFO* si = new STARTUPINFO();
			//PROCESS_INFORMATION* pi = new PROCESS_INFORMATION();
			//CreateProcess(NULL, const_cast<char *>(constr.str().c_str()), NULL, NULL, false, NULL, NULL, NULL, si, pi);
		}
	}
